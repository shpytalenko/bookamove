class PersonalCalendarsController < ApplicationController
  ...
  def title_page
    @title_page = 'Personal Calendar'
  end

  def personal_calendar_event
    staff_subtask = []
    move_record = []
    ...
    if (params[:available].present?)
      ...
      subtask_group = TaskAvailableCalendar.where("DATE(start_time) BETWEEN ? AND ? AND account_id = ? ", params[:start], params[:end], @current_user.account_id)
                          .where(user_id: @current_user.id).includes(:subtask_staff_group)
      staff_available = StaffAvailable.where("DATE(start_time) BETWEEN ? AND ? AND account_id = ? ", params[:start], params[:end], @current_user.account_id)
                            .where(user_id: @current_user.id)
      reminder = ReminderCalendarPersonal.where("DATE(date) BETWEEN ? AND ? AND account_id = ? ", start_date_reminder, end_date_reminder, @current_user.account_id)
                     .where(user_id: @current_user.id)
    ...
    subtask_group.each do |list_subtask|
      staffavailable.push(
          start: list_subtask.start_time.to_s,
          :end => list_subtask.end_time.to_s,
          name: list_subtask.subtask_staff_group.name,
          className: 'available-task-calendar',
          editable: false,
          taskAvailable: list_subtask.id,
          type: 'task-available',
          overlap: false,
          task_available_id: list_subtask.id)
    end

    unavailable = []
    staff_available.each do |available_time|
      unavailable.push(
          start: available_time.start_time.to_s,
          :end => available_time.end_time.to_s,
          rendering: 'background',
          className: 'opacity-event ' + (available_time.available ? 'available-calendar' : 'unavailable-calendar'),
          editable: false,
          type: 'background')
    end

    respond_to do |format|
      format.json { render json: staffavailable + movers + unavailable + reminders }
  ...
  def add_reminder_calendar_personal
    respond_to do |format|
      reminder = ReminderCalendarPersonal.new
      reminder.account_id = @current_user.account_id
      reminder.user_id = params[:staff_id].present? ? params[:staff_id] : @current_user.id
      reminder.author = @current_user.id
      reminder.date = params[:date]
      reminder.messages_personal_calendar_id = params[:message_staff_calendar_id]
      if reminder.save
  ...
  def destroy_reminder_calendar_personal
    respond_to do |format|
      reminder = ReminderCalendarPersonal.find_by_id(params[:reminder])
      if reminder.destroy
        format.json { render json: reminder }
      else
        format.json { render json: reminder.errors }
  ...
  def index_messages
    @all_messages = MessagesStaffAvailableCalendar.where(account_id: @current_user.account_id, :staff_id => @current_user.id)
