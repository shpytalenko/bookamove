...
(function (factory) {
    ...
    var defaults = {
        ...
        // time formats
        titleFormat: {
            month: 'MMMM', // like "September 1986". each language will override this
            week: 'll', // like "Sep 4 1986"
            day: 'MMMM DD' // like "September 4 1986"
        },
        columnFormat: {
            month: 'ddd', // like "Sat"
    ...
    function Header(calendar, options) {
        ...
        function render() {
            ...
            if (sections) {
                ...
                $("#calLeft").html(renderSection('left', 'notd'));
                $("#calRight").prepend(renderSection('right', 'notd'));
                return element;
            }
        }
        ...
        function renderSection(position, opts) {
            if (opts != "notd") {
                var e = $("<td class='fc-header-" + position + "'/>");
            }
            else {
                var e = $("<div class='btn btn-none'/>");
            }
            ...
            if (buttonStr) {
                $.each(buttonStr.split(' '), function (i) {
                    ...
                    $.each(this.split(','), function (j, buttonName) {
                        if (buttonName == 'title') {
                            e.append("<span class='fc-header-title'><h2>&nbsp;</h2><div class='reminder-monthly'></div></span>");
                            if (prevButton) {
        ...
        function _fetchEventSource(source, callback) {
            ...
            if (events) {
                ...
                if (url) {
                    ...
                    /* oomover reminder-time*/
                    if (t.getView().name === 'resourceDay') {
                        data['date_reminder'] = t.getDate().format();
                    }
...
// Break the formatting string into an array of chunks
    ...
    function BasicView(element, calendar, viewName) {
        ...
        function buildTableHTML() {
            var html =
                "<table class='fc-border-separate' style='width:100%; height: 450px' cellspacing='0'>" +
                buildHeadHTML() +
                buildBodyHTML() +
                "</table>";
        ...
        function buildSkeleton() {
            ...
            while (slotTime < maxTime) {
                ...
                s +=
                    ...
                    "<th class='fc-agenda-axis " + headerClass + "'><div class='move-top-17'>" +
                    ((!slotNormal || !minutes) ?
        ...
        function buildDayTableHeadHTML() {
            ...
            for (col = 0; col < colCnt; col++) {
                ...
                html +=
                    ...
                    "<span class='reminder-weekly' data-reminder-time='" + date.format('YYYY-MM-DD') + "'></span><br>" +
                    "<span>" + htmlEscape(formatDate(date, colFormat)) + "</span>" +
    ...
    function AgendaEventRenderer() {
        ...
        function renderSlotSegs(segs, modifiedEventId) {
            ...
            for (i = 0; i < segCnt; i++) {
                seg = segs[i];
                if ((eventElement = seg.element)) {
                    eventElement[0].style.width = ((Math.max(0, seg.outerWidth - seg.hsides)) + 5) + 'px';
        ...
        function slotSegHtml(event, seg) {
            ...
            if (event.type == 'move_record') {
                var name_trucks = '<div class="truck-event-description">Truck: ' + event.truck_names.truck + ' , ' + 'Movers: ' + (event.truck_names.movers === "" ? 'Empty' : event.truck_names.movers ) + '</div>';
                html +=
                    " class='" + classes.join(' ') + "'" +
                    " style=" +
                    "'" +
                    ...
                    "<div class='start-travel'>Begin travel: " + event.half_travel + " hour(s)</div>" +
                    "<div class='fc-event-time'>" +
                    htmlEscape(t.getEventTimeText(event)) +
                    "</div>" +
                    "<div class='fc-event-title'>" +
                    "<i class='icon-calendar'></i>&nbsp;" + ( typeof event.url_move == 'string' ? "<a href='" + event.url_move + "' target='_self'>" : '<span>' ) +
                    htmlEscape(event.title || '') +
                    ( typeof event.url_move == 'string' ? "</a>" : '</span>' ) +
                    name_trucks +
                    "</div>" +
                    "<div class='end-travel'> Return travel: " + event.half_travel + " hour(s)</div>" +
                    "</div>" +
                ...
                html +=
                    ...
                    "</div>" +
                    "<div class='fc-event-bg'></div>";
            } else if (event.type === 'task-available') {
                html +=
                    " class='" + classes.join(' ') + "'" +
                    " style=" +
                    "'" +
                    ...
                    skinCss +
                    "z-index:100;'" +
                    " data-task='" + event.taskAvailable + "'>" +
                    "<div class='fc-event-inner'>" +
                    "<div class='fc-event-time'>" +
                    htmlEscape(t.getEventTimeText(event)) +
                    "</div>" +
                    "<div class='fc-event-title'>" +
                    '<i class="icon-male"></i> ' + event.name +
                    "</div>" +
                    "</div>" +
                    "<div class='fc-event-bg'></div>";
            } else {
...
// coordinate of the first segment in the series.
    function computeSlotSegCoords(seg, seriesBackwardPressure, seriesBackwardCoord) {
        ...
        if (seg.forwardCoord === undefined) { // not already computed
            ...
            seg.backwardCoord = 0; //oomovers-overlay-available
            seg.forwardCoord = 1;//oomovers-overlay-available
...
// TODO: test liquid width, especially in IE6
    ...
    function ResourceView(element, calendar, viewName) {
        ...
        function buildSkeleton() {
            ...
            slotSegmentContainer =
                $("<div class='fc-event-container' style='position:absolute;top:0;left:0'/>")
                    .appendTo(slotContainer);
        ...
        function buildDayTableHeadHTML() {
            ...
            for (col = 0; col < colCnt; col++) {
                ...
                html +=
                    ...
                    (resource.url == undefined ? htmlEscape(resource.name) : '<a href="' + resource.url + '&date_calendar=' + calendar.getDate().format('YYYY-MM-DD') + '" target="_self">' + htmlEscape(resource.name) + '</a>')
                    + "<span class='reminder-resource-name'></span>" +
                    "</th>";
            }
    ...
    function ResourceEventRenderer() {
        ...
        function compileSlotSegs(events) {
            ...
            reminder_monthly_html(events);
            $('.fc-widget-header .reminder-resource-name').html('');

            return segs;
        }

        function reminder_monthly_html(events) {
            /*oomovers - monthly reminder*/
            var reminder_monthly_html = '';
            for (var i = 0; i < events.length; i++) {
                if (events[i].type === 'reminder' && events[i].resources.length === 0) {
                    reminder_monthly_html += "<div class='pull-left has-popover' data-content='" + events[i].comment +
                        "' data-placement='top' data-title='" + events[i].subject +
                        "' data-original-title='" + events[i].subject +
                        "' data-container='body' data-selector='i.icon-time' data-html=true >" +
                        "<i class='icon-time' style='float: right; margin: 3px;'></i>" +
                        "</div>"
                }
            }
            $('.reminder-monthly').html(reminder_monthly_html);
        }
        ...
        function slotSegHtml(event, seg) {
            ...
            //custom by oomovers - resourceView-oomovers
            if (event.type == 'move_record') {
                var name_trucks = '<div class="truck-event-description">Truck: ' + event.truck_names.truck + ' , ' + 'Movers: ' + (event.truck_names.movers === "" ? 'Empty' : event.truck_names.movers ) + '</div>';

                html += " class='" + classes.join(' ') + " move-record-droppable";

                if ($.inArray("Receive", event.all_move_stages) != "0" || $.inArray("Confirm", event.all_move_stages) != "0" || $.inArray("Dispatch", event.all_move_stages) != "0") {
                    html += " special-stage"
                }
                ...
                html += "' style=" +
                    ...
                    skinCss +
                    "z-index:100;'" +
                    " data-move='" + event.move_id + "'>";

                if (event.all_info_blacked_out){
                    html += "<div class='fc-event-inner' style='background-color: #000 !important;'> " +
                            "<div class='fc-event-title'> ";
                    html += "</div></div>";
                    html += "</div>" +
                            "<div class='fc-event-bg'></div>";
                    ...
                    var allAlerts;
                    for (var key in event.all_alerts) {
                        if (event.all_alerts.hasOwnProperty(key)) {
                            allAlerts += "<div>" + key + " <span class=textOrange>" + event.all_alerts[key] + "</span></div>";
                        }
                    }
                    var nameLimit = 20;
                    if (seg.outerHeight < 61) {
                        html += "<span class='pull-right' style='padding-right: 1.5%;'><i class='fa fa-plus-circle sfont' data-toggle='modal' data-target='#" + event.move_id + "Modal'></i></span>";
                        nameLimit = 6;
                    }

                    html += "<div class='sln2'><a href='" + event.url_move + "' target='_self'>" + htmlEscape(event.title.toString().substring(0, nameLimit) || '') + "</a> <span class='textOrange'>(" + event.current_move_stage + ")</span></div>";
                    html += "<div class='sln'>" + htmlEscape(event.what) + "</div>";
                    html += "<div class='sln'>" + htmlEscape(event.where) + "</div>";
                    html += "<div class='sln'><span class='textOrange'>" + htmlEscape(event.current_alert || '') + "</span>" +
                        "<a href='javascript:void(0)' data-container='body' data-toggle='popover' data-placement='bottom' " +
                        " data-content='" + allAlerts.replace("undefined", "") + "' data-html='true'><span class='caret'></span></a>";
                    //name_trucks +
                    html += "</div></div>";
                    ...
                    $('#modal').append('<div id="' + event.move_id + 'Modal" class="modal fade" role="dialog">' +
                        '<div class="modal-dialog">' +
                        '<!-- Modal content-->' +
                        '<div class="modal-content">' +
                        '<div class="modal-header">' +
                        '<button type="button" class="close" data-dismiss="modal">&times;</button>' +
                        '<h4 class="modal-title">' + htmlEscape(event.title) + ' <span class="textOrange">(' + event.current_move_stage + ')</span> </h4>' +
                        '</div>' +
                        '<div class="modal-body">' +
                        '<p>' +
                        '<div>' + htmlEscape(event.what) + '</div>' +
                        '<div>' + htmlEscape(event.where) + '</div>' +
                        '<div>' + allAlerts.replace("undefined", "") + '</div>' +
                        '</p>' +
                        '</div>' +
                ...
                var temp_reminder_html = "<div class='has-popover pull-right' data-content='" + event.comment +
                    "' data-placement='top' data-title='" + event.subject +
                    "' data-original-title='" + event.subject + "' data-container='body' data-selector='i.icon-time' data-html=true >" +
                    "<i class='icon-time' style='margin:3px;'></i>" +
                    "</div>";
                html += '>';
                $('.fc-widget-header.truck-id-' + event.resources[0] + ' .reminder-resource-name , .fc-widget-header.task-id-' + event.resources[0] + ' .reminder-resource-name').append(temp_reminder_html);
            } else if (event.type === 'background') {
                html +=
                    " class='" + classes.join(' ') + "'" +
                    " style=" +
                ...
                html +=
                    " class='" + classes.join(' ') + " task-available-droppable'" +
                    " style=" +
                    "'" +
                    "position:absolute;" +
                    "top:" + seg.top + "px;" +
                    ...
                    "<div class='fc-event-inner'>" +
                        ...
                        //htmlEscape(t.getEventTimeText(event)) +
                        //"</div>" +
                    "<div class='fc-event-title' style='padding-top:2%'>" +
                    '<i class="icon-male"></i> ' + event.name
                        if (event.description) {
                            html += "<br class='mtop5'/>Description: " + event.description
                        }
                        if (event.notes) {
                            html += "<br class='mtop5'/>Notes: " + event.notes
                        }
                    html += "</div>" +
                    "</div>" +
                    "<div class='fc-event-bg'></div>";

                //<!-- staff cal event full details modal -->
                $('#modal').append('<div id="' + event.task_available_id + 'Modal" class="modal fade" role="dialog">' +
                    '<div class="modal-dialog">' +
                    '<!-- Modal content-->' +
                    '<div class="modal-content">' +
                    '<div class="modal-header">' +
                    '<button type="button" class="close" data-dismiss="modal">&times;</button>' +
                    '<h4 class="modal-title"><i class="icon-male"></i> ' + htmlEscape(event.name) + '</h4>' +
                    '</div>' +
                    '<div class="modal-body">' +
                    '<p>' +
                    '<div class="form-group"><label>Description: </label><input type="text" class="form-control" id="edit_sf_description" name="edit_sf_description" value="'+ (event.description || "") +'"></div>' +
                    '<div class="form-group"><label>Notes: </label><input type="text" class="form-control" id="edit_sf_notes" name="edit_sf_notes" value="'+ (event.notes || "") +'"></div></div>' +
                    '</p>' +
                    '</div>' +
                    '<div class="modal-footer">' +
                    '<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>' +
                ...
                stop: function (ev, ui) {
                    ...
                    if (isInBounds && (isAllDay || resourceDelta || snapDelta)) { // changed!
                        ...
                        if (resourceDelta) {
                            ...
                            //custom by oomovers
                            event.oldResource = oldId;
                            event.newResource = newId;
                        }
                        eventDrop(
                            eventElement[0],
                            event,
    ...
    function DayEventRenderer() {
        ...
        var calendar = t.calendar;
        var getEventEnd = calendar.getEventEnd;
        t.minTime = moment.duration(opt('minTime'));
        t.maxTime = moment.duration(opt('maxTime'));
        t.slotDuration = moment.duration(opt('slotDuration'));
        ...
        function _renderDayEvents(events, doAppend, doRowHeights) {
            ...
            segmentElementEach(segments, function (segment, element) {
                if (segment.event.type == "reminder") {
                    element.width(
                        15 //custom oomovers
                    );
                } else {
                    element.width(
                        Math.max(0, (segment.outerWidth - segment.hsides) / 7) //custom oomovers
                    );
                }
            });
        ...
        function calculateHorizontals(segments) {
            ...
            for (var i = 0; i < resources.length; i++) {
                resources[i].id !== '' ? temp_resources[resources[i].id] = i + 1 : '';
            }
            $('body').removeData();
            ...
            for (var i = 0; i < segments.length; i++) {
                ...
                /*custom oomovers*/
                if (segment.event.type == "reminder") {
                    /*var reminder_length = $('[data-reminder-date="'+ segment.event.start.format('DD/MM/YYYY') + '"]').length;
                     */
                    var reminder_length = $('body').data(segment.event.start.format('DD/MM/YYYY'));
                    if (reminder_length === undefined) {
                        $('body').data(segment.event.start.format('DD/MM/YYYY'), []);
                    }
                    segment.left = left + (15 * ($('body').data(segment.event.start.format('DD/MM/YYYY')).length + 1));
                    $('body').data(segment.event.start.format('DD/MM/YYYY')).push(1);

                } else {
                    segment.left = left + ((segment.outerWidth / 6) * temp_resources[segment.event.resources[0]]) - (segment.outerWidth / 6);
                }
            }
        }
        ...
        function buildHTMLForSegment(segment) {
            ...
            if (event.type === 'background') return '<div></div>';

            if (event.type === 'reminder') {
                var reminder_height = $('.fc-first > div:first .fc-day-number').height();
                html += "<div style='position:absolute;background: transparent;" +
                    "left:" + segment.left + "px;" +
                    "height: " + reminder_height + "px;' class='fc-event-inner has-popover pointer' data-content='" + event.comment +
                    "' data-placement='top' data-reminder-date='" + event.start.format('DD/MM/YYYY') + "'data-title='" + event.subject +
                    "' data-original-title='" + event.subject + "' data-html=true >" +
                    "<i class='icon-time'></i>" +
                    "</div>";
            } else {
                ...
                var height_mover = (moment.duration(event.end) - moment.duration(event.start)) / t.slotDuration;
                height_mover = Math.floor(height_mover);
                var top_mover = ($('.fc-first > div:first').height() - $('.fc-first > div:first .fc-day-number').height()) / ((t.maxTime - t.minTime) / t.slotDuration);
                height_mover = (height_mover * top_mover) - 1;
                html +=
                    " class='" + classNames.join(' ') + "'" +
                    " style=" +
                    "'" +
                    ...
                    "left:" + segment.left + "px;" +
                    skinCss +
                    "height: " + height_mover + 'px; ' //oomovers top calculation
                "'" +
                ">";
                html += "<div class='fc-event-inner'>";
                ...
                /*custom by oomovers monthView-oomovers*/
                if (event.type === 'move_record') {
                    html +=
        ...
        function setVerticals(segments, doRowHeights) {
            ...
            segmentElementEach(segments, function (segment, element) {
                element.css(
                    'top',
                    rowContentTops[segment.row] + segment.top // ...now, relative to views's origin // custom oomovers
                );
            });
        }
        ...
        function calculateVerticals(segments) {
            ...
            for (var rowI = 0; rowI < rowCnt; rowI++) {
                ...
                for (var segmentI = 0; segmentI < segmentRow.length; segmentI++) {
                    ...
                    if (segment.event.type == "reminder") {
                        segment.top = (segment.outerHeight * -1);
                    } else {
                        segment.top = calculationOomoverTop(segment.event.start); //oomovers calculation
                    }
        ...
        //oomovers calculation
        function calculationOomoverTop(startDate) {
            var time = moment.duration(startDate);
            var slots = (time - t.minTime) / t.slotDuration;
            var slotIndex = Math.floor(slots);
            var top_mover = ($('.fc-first > div:first').height() - $('.fc-first > div:first .fc-day-number').height()) / ((t.maxTime - t.minTime) / t.slotDuration);
            return slotIndex * top_mover;
        }
...
// A cmp function for determining which segments should appear higher up
    function compareDaySegments(a, b) {
        return (b.rightCol - b.leftCol) - (a.rightCol - a.leftCol) || // put wider events first
            ...
            (a.event.title || ''); // if a tie, sort by event title
    }
