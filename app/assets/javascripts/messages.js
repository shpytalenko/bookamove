$(document).ready(function () {
    ...
    $('.message-inbox, .dashboard-home').on('click', '.non-read, .non-read', function () {
        var non_read = $(this);
        var message_id = non_read.data('message');
        var message_type = non_read.data('type-message');
        $.ajax({
                url: 'update_readed_message.json',
                type: 'PUT',
            ...
            .done(function (data) {
                ...
                //if ($('.notification-message-' + message_id).length > 0) {
    ...
    $('.send-new-message').on('click', function () {
        var multiselect = $(".new-message .multi-select-new-message-to");
        var body = $(".new-message .message-body").val();
        var required = false;
        ...
        var subject = $(".new-message .message-subject").val();

        var urgent = $(".new-message .message-priority").val();
                ...
                buttons: {
                    ...
                    primary: {
                        ...
                        callback: function () {
                            ...
                            save_message(form_attachment_message);
    ...
    /*$('.message-inbox .reply-message, .dashboard-home .reply-message').on('click', function(){
     $('.reply').remove();
     var textarea_body = $('.new-message .message-body').clone();
     ...
     $( ".message-inbox, .dashboard-home" ).on('click','.cancel-new-reply', function(){
     var message_id = $(this).data('message');
     $('.message-inbox .wrap-message-' + message_id).remove();
     });

     $( ".message-inbox, .dashboard-home" ).on('click','.send-new-reply', function(){
     var message_id = $(this).data('message');
    ...
    $('.message-inbox .attach-file-message').change(function (evt) {
        handleFileUpload(evt.target.files);
    });

    $('.message-inbox, .dashboard-home').on('click', '.redirect_move_record', function (event) {
        window.location = $(this).attr('href');
    });

});

function mark_message(message_id, mark_type, unmark) {
    $.ajax({
            url: '/mark_message.json',
            type: 'PUT',
...
function save_message(form_attachment_message) {
    $.ajax({
            url: '/messages.json',
            type: 'POST',
            data: form_attachment_message,
        ...
        .done(function (data) {
            ...
            $(".message-inbox .star-priority").addClass('icon-star-empty').removeClass('icon-star').css({"color": "black"});
